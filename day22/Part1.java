...
public class Part1 {
    ...
    private static int analyseBlocks(ArrayList<Block> blocks) {
        int sum = 0;
        for (Block block : blocks) {
            boolean canRemove = true;
            for (Block supportFor : block.getSupportFor()) {
                if (supportFor.getSupportedBy().size() == 1) {
                    canRemove = false;
                    break;
                }
            }
